...
function DesktopStep({ number, icon, title, description, isLast = false }: StepProps) {
  return (
    <div className="flex flex-col items-center relative flex-1">
      ...
      {!isLast && (
        <div className="absolute top-7 left-1/2 w-[calc(100%+1rem)] h-0.5 border-t-2 border-dashed border-fiber-orange/30 z-0" />
      )}

      {/* Number badge circle */}
      <div className="flex items-center justify-center w-14 h-14 rounded-full bg-fiber-orange text-white font-bold text-lg mb-4 relative z-10 flex-shrink-0">
        {number}
      </div>
...
function MobileStep({ number, icon, title, description }: StepProps) {
  return (
    <div className="flex gap-6 relative">
      ...
      <div className="absolute left-6 top-14 w-0.5 h-[calc(100%+1rem)] bg-fiber-orange/30" />

      {/* Number badge and icon */}
      <div className="flex flex-col items-center flex-shrink-0 relative z-10">
        <div className="flex items-center justify-center w-14 h-14 rounded-full bg-fiber-orange text-white font-bold text-lg mb-4">
          {number}
        </div>
